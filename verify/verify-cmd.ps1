...
foreach ($f in @("main.go","model.go","msg/types.go","pages/menu.go","pages/config_backup.go","pages/config_restore.go","pages/progress.go","pages/result.go","shared/input.go","shared/disk_list.go","shared/progress_bar.go")) {
    $path = "$root\$pkg\$f"
...
if ($ok) { Write-Host "`n  CMD — 正常" -ForegroundColor Green } else { Write-Host "`n  CMD — 不正常" -ForegroundColor Red }
exit $(if ($ok) {0} else {1})
